    ...
    <header class="detail_header" :style="{color: Color}">
      ...
      <div class="bar">
        <div class="topBar">
          ...
          <div class="factory" title="收藏此文章" @click="factory(article_id)">
            <i class="ivu-icon ivu-icon-md-star"></i>
          </div>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    ...
    getnotedetail(`/note/bynotetext/${this.$route.params.id}`).then((res) => {
      ...
      this.username = baseURL.content[0].username;
      this.article_id = baseURL.content[0].article_id;
  ...
  methods: {
    ...
    factory(article_id) {
      ...
      if (username) {
        ...
        PostMessage("note/factory", obj).then((res) => {
          if (res.data.err == 0) {
